...
class Boost:
    ...
    def execute_Boost_test_default_novbso(self):
        startup_procedure = self.procedures['Boost_Test_Default_Novbso'].loc[0].split('\n')
        for instruction in startup_procedure:
            instruction = instruction.lower()
            if re.match('0x', instruction):
                reg_data = self.parser.extract_RegisterAddress__Instruction(instruction)
    ...
    def boost_DFT(self,data=pd.DataFrame({}), test_name=''):
        ...
        for instruction in instructions:
            ...
            if re.match('run', instruction):
                ...
                if re.findall('Boost_test_default_novbso'.lower(), instruction):
                    print('Enable Boost Test Default Procedure No VBSO')
                    self.execute_Boost_test_default_novbso()
...
if __name__ == '__main__':
    ...
    try:
        for test in tests.Boost:
            ...
            boost.current_priority_set = False
            sleep(0.1)
            boost.power_off()
            sleep(0.2)
            boost.power_on()
