...
class AZ_comp:
    ...
    def DFT_Tests(self,path='Tests.yaml'):
        return az_comp.read_yaml(path_to_yaml=path)
    ...
    def convert_dict_values(self,data):
        ...
        for key, value in data.items():
            converted_value = AZ_comp.convert_value_unit(value)
    ...
    def execute_startup(self):
        ...
        for instruction in startup_procedure:
            ...
            if re.match('0x', instruction):
                reg_data = self.parser.extract_RegisterAddress__Instruction(instruction)  # Usa self
                self.write_device(reg_data)  # Usa self
            if re.match('Force__SDWN__1.8V'.lower(), instruction):
                print('Force 1.8V on SDWN')
                self.mcp2317.Switch(device_addr=0x20, row=1, col=4, Enable=True)  # Usa self

    def write_device(self,data: {}):
        device_data = self.mcp.mcpRead(SlaveAddress=AZ_comp.slave_address, data=[int(data.get('RegAddr'), 16)])[0]
        ...
        if int(data.get('Data'), 16) < bit_width:
            ...
            self.mcp.mcpWrite(SlaveAddress=AZ_comp.slave_address, data=[int(data.get('RegAddr'), 16), device_data])
        else:
            print(f'Data is out of width')

    def execute_Enable_Ana_Testpoint(self):
        startup_procedure = AZ_comp.procedures['Enable_Ana_Testpoint_AZ_comp'].loc[0].split('\n')
        for instruction in startup_procedure:
            instruction = instruction.lower()
            if re.match('0x', instruction):
                reg_data = self.parser.extract_RegisterAddress__Instruction(instruction)
                AZ_comp.write_device(reg_data)
    ...
    def AZcomp_DFT(self,data=pd.DataFrame({}), test_name=''):
        ...
        typical = AZ_comp.typical_value_clean(data[test_name].loc[6])
        print(typical)
        for instruction in instructions:
            ...
            if re.match('run', instruction):
                if re.findall('startup', instruction):
                    ...
                    AZ_comp.execute_startup()
                if re.findall('Enable_Ana_Testpoint_AZ_comp'.lower(), instruction):
                    print('Enable Ana TestPoint Procedure')
                    AZ_comp.execute_Enable_Ana_Testpoint()
            if re.match('force', instruction):
                force_signal_instruction = self.parser.extract_ForceSignal(instruction)
                print(f'Force Signal : {force_signal_instruction}')
                AZ_comp.force_signal(force_signal_instruction)
            if re.match('measure', instruction):
                measure_signal = self.parser.extract_measureSignal(instruction)
                print(f'Measure Signal : {measure_signal}')
                AZ_comp.measure_value_check(measure_signal=measure_signal, typical=typical)
